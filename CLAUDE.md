...
## 24. ✅ BATCH ORCHESTRATOR (BUILT + VALIDATED 2026-07-17) — `orchestrator/`
The §20 conveyor belt, now real. Python package chaining, per part:
**downloaded → extracted (OCR+reconcile) → loaded (Postgres) → archived → done**, state-by-state but **clearing local disk per AC**. Resumable at part granularity; a failure flags the part and moves on.

**Files:** `orchestrator/jobs.py` (`JobStore` — extends the scraper's `download_job` in `manifest.db` with pipeline columns via idempotent migration: `pipeline_stage`, `reconciles`, `elector_count`, `pipeline_error`, `archive_uri`, `pipeline_updated_at`), `orchestrator/db.py` (`PgLoader` — runs `ocr.load.to_sql` output through a configurable `psql` client over stdin; works against a local server, a URL, or `docker exec -i ems_pg psql …`), `orchestrator/archive.py` (`Archiver` — `local` dir copy now; `none`; rclone/S3/OneDrive are the same `put(rel_key, src)` contract later), `orchestrator/run.py` (the loop + CLI).

**Run:** `scraper/.venv/bin/python -m orchestrator.run --state Lakshadweep --pg '<psql cmd>' --archive-dir <dir> [--acs 1,2] [--clear] [--deleted-backend dp] [--dpi 200] [--download] [--require-reconcile] [--force]`.

**Safety invariants (built in):** a part is cleared from local disk ONLY after it is both loaded AND archived; `--clear` is OFF by default (keeps files); `--clear` + `--archive-backend none` is **refused** (never delete the only copy). Non-reconciling parts still LOAD by default (data is present) and are flagged (`reconciles=0`); `--require-reconcile` skips their load instead.

**✅ VALIDATED end-to-end on Lakshadweep U06 AC1 (all 10 parts, real Postgres 16 via Docker):** done=10/10, extract→reconcile→load→archive all worked; 8/10 reconcile (parts 3&5 = the known source-PDF gender defects, flagged not failed); DB ended with **10 rolls, 8995 electors, 8975 persons, 8995 placements, 10 cover_counts** — the 8995→8975 gap = **20 people appearing in 2+ parts of the AC, correctly collapsed to one person each** (identity model proven across a whole real AC). `cover_matches_summary=t` on all 10 (cover second-checksum holds). Verified: **resume** (re-run skips all 10 in 0s, DB stays 10 rolls — idempotent), the **clear-safety refusal**, and the **--clear path** (on throwaway copies: local deleted only after archive+load; archive preserved). ~150s/part on the Mac (RapidOCR, `--deleted-backend none`).

**⚠️ Remaining before a real state run:** (1) **download integration is a stub** — `_ensure_local` shells to the Node `bin/download.js` (which walks a whole STATE, not per-AC) and needs the **ECI portal reachable** (currently broken, §0) or a CEO-site path; true per-AC download→process→clear pipelining wants an `--ac` flag on `bin/download.js`. (2) **`--force` reload isn't idempotent** — `roll` insert has no on-conflict, so re-loading an already-loaded roll violates `source_pdf_sha256`/`uq_roll_natural`; normal resume avoids this via the `pipeline_stage=done` skip, but harden the roll insert (on-conflict) before relying on `--force`. (3) OneDrive/S3 archive backend (only `local` copy built). (4) Co-locate on the AWS box for the real batch (scraper needs Indian egress, PII stays in-region); measure GB + GPU/CPU-hours per state on batch-1 to project the national bill.
