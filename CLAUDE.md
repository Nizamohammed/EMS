...
**Captcha solver plan (NOT the agent at scale):** pluggable `CaptchaSolver` (mirrors the OCR extractor swap-point). Default = **local `qwen2.5vl:3b` (Ollama)** — the "ML model in the loop" the user asked for. Smoke test: read `u0k5jg` vs true `uak5jg` = 5/6 chars (missed strikethrough-crossed `a`). Viable anyway because **retry-on-failure + the Success oracle** make modest per-solve accuracy cheap over ~thousands of solves. Boosters before relying on it: de-strikethrough preprocessing, multi-read voting, Tesseract/audio-captcha ensemble. Agent = dev/bootstrap/escalation only. Benchmark candidates on a harvested batch before committing.

**Files:** `scraper/lib/{api,manifest,browser}.js`, `scraper/bin/{crawl-manifest,probe-captcha-scope,enumerate-parts}.js`.

**Robot loop / orchestrator (BUILT & PROVEN end-to-end on Lakshadweep):** decision (user) = **lazy per-AC enumeration**, not a separate up-front pass — enumerating an AC's parts and downloading them use the SAME page visit, so the robot reads the part list, seeds the parts `pending`, then downloads in one go (no double-visit, no staleness). Files: `scraper/lib/download.js` (`downloadAc`: cascade → `readParts`+seed pending → select not-yet-verified parts → solve 1 captcha (retry-on-reject by reloading) → wait per-UUID downloads → verify `%PDF`+`pdfinfo` page count + sha256 → `markJob` verified/failed), `scraper/lib/captcha.js` (pluggable `getSolver('manual'|'model')` — manual=file-handoff bootstrap, model=local `qwen2.5vl:3b`), `scraper/bin/download.js` (driver: discover AC coords → run robot per AC). **Proven:** Lakshadweep U06 ENG → 10 parts, 1 captcha, **10/10 downloaded + verified** (distinct sha256, page counts 12–41), to `scraper/data/rolls/U06/1/`; **part-level resume works** (re-run → 0 downloads, 0 captchas, skips verified). The `enumerate-parts.js` standalone seeder still exists (enumeration-only / `ac_availability` menu).

**Immediate next:** (a) **swap in the model solver at scale** — `--solver model` is wired; benchmark `qwen2.5vl:3b` exact-match rate on a harvested captcha batch + add de-strikethrough preprocessing/voting, then run unattended (retry loop + Success oracle already handle misses); (b) **harden multi-AC/multi-district enumeration** — `selectAcByIndex` index-walk is robust for single-AC states but needs type-to-filter for 200–400-AC states; then scale across states; (c) test whether one captcha covers a *large* AC selection (300+ parts) or if batching is needed; then feed `scraper/data/rolls/` PDFs into the `ocr/` pipeline.
