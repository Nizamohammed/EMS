...
- **Loader robustness fixes (same session, needed for multi-part batch loads):** geography inserts (district/PC/AC/part/section) are now idempotent **upserts** (`on conflict … do update … returning` / `do nothing`) — a state batch loads thousands of parts under one district/AC; plain inserts collided. And the PC is only created when a valid `pc_no` is read (cover often misses it) → `ac.pc_id` left NULL instead of fabricating `pc_no=0` (which fails the `>=1` check and crashed the roll). ⚠️ Still cover-extraction gaps (surface as `year_of_revision=0`, `pc` NULL): the RapidOCR cover parser is best-effort — improve `_parse_cover` (or fall back to the filename/voter-page header for PC no) before relying on cover fields.
